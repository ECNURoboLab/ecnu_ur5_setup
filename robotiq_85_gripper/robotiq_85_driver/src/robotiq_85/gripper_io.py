...
class GripperIO:
    def __init__(self,device):
        ...
        self.act_cmd[:7] = [self.device, 0x10, 0x03, 0xE8, 0x00,0x08, 0x10] # we will send 16 bytes at payload
        self.act_cmd_bytes = ""
        self._update_cmd()
        self.stat_cmd = [self.device, 0x03, 0x07, 0xD0, 0x00, 0x08]
        compute_modbus_rtu_crc(self.stat_cmd)
        self.stat_cmd_bytes = array.array('B',self.stat_cmd).tostring() # for asking the gripper states
    ...
    def goto(self, pos, vel, force):
        self.rACT = 1
        self.rGTO = 1
        self.rPR = int(np.clip((3.-230.)/0.085 * pos + 230., 0, 255)) # for 0 the gripper will fully open and for 255 the gripper will fully close
