...
class Robotiq85GripperTest:
    ...
    def _run_test(self):
        ...
        while not ready:
            """
            wait the gripper to be ready
            """
            ready = True
            ready = self._gripper_stat[0].is_ready
        self._gripper_cmd[0].position = 0.00
        while not rospy.is_shutdown():
            # send the req to gripper
            keyin = raw_input("send one msg :(y/n)")
            if keyin == 'y':
                rospy.loginfo("send one msg to gripper")
                self._gripper_cmd[0].position += 0.02
                self._gripper_cmd[0].speed = 0.015
                self._gripper_cmd[0].force = 100.0
